...
mdb.models['ExplicitModel'].DisplacementBC(amplitude='Amp-1', createStepName=
    ...
    u3=SET, ur1=SET, ur2=Load_Angle, ur3=SET)
...
jobName_Explicit = 'CTLTs_Y_Explicit'
mdb.Job(model=ExplicitModel, name=jobName_Explicit, numCpus=10, numDomains=10, numGPUs=1, explicitPrecision=
...
outfile.write('Diaplacement UR2 [rad]' + ',' + 'Reaction Moment RM2 [Nm]' + '\n')
...
for fm in range(0, len(odb.steps[stepName].frames)):
  ...
  Displacement[fm] = readNodeDispValues[0].dataDouble[1] # 0-X Direction; 1-Y Direction; 2-Z Direction
  ReactionForce[fm] = readNodeRFValues[0].dataDouble[1]/1000.0
